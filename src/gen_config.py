...
def gen_train(path, device, use_physics=False, dataset="spherical_shell", d=0.2, train_ratio=None):
...
def gen_test(path, dataset="spherical_shell", d=0.2, use_physics=False):
    if not os.path.exists(path):
        os.mkdir(path)
    for model in MODELS:
        config = {
...
if __name__ == '__main__':
    ...
    gen_train("./config/train_rectangle_pinn", args.device, use_physics=True, dataset="rectangle", d=0.1)
    gen_test("./config/scale_variant_test_rectangle_pinn",  dataset="rectangle", d=0.05, use_physics=True)
    gen_test("./config/scale_invariant_test_rectangle_pinn", dataset="rectangle", d=0.1, use_physics=True)
